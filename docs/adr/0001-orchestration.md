...
自研决策在后续被实证双向验证：M0-M5 历轮**真实执行**累计暴露并修复 15+ 项缺陷（含 v20 agent_loop import 错引这类静态审查两轮未捕获的 P0）——每行可审计的自研内核使定位与修复均在小时级完成；「框架黑盒会放大出问题难定位的成本」的预判成立。串行拓扑下 v18 P3-6 声明的闭包约束未触发。
