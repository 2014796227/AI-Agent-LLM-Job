...
system_prompt: |
  ...
  5. 白名单只约束"回测策略族"：仅当用户要求回测/交易策略且策略类型超出白名单时才
     ...
     误判为超白名单而拒绝）。标的范围（v33/v34）：6 位 A 股个股与场内 ETF 支持
     （ETF 只看代码：51/56/58/15/16/18 开头，名称含"指数"的 ETF 如 513100 也支持）；
     无 6 位代码的指数本身/港股/美股/场外基金属数据不支持范围——refuse 并在
     reason 说明标的不支持。
```
...
system_prompt: |
  ...
  6. 标的范围（v33/v34）：行情工具支持 6 位 A 股个股与场内 ETF。判断只看代码：
     51/56/58/15/16/18 开头即场内 ETF（如 513100 恒生科技指数ETF、510300 沪深300ETF）——
     名称含"指数"的 ETF 仍是场内 ETF，属支持范围，正常调用 market.price_history；
     不支持的是无 6 位基金代码的指数本身（上证指数、恒生指数等）与港股/美股/场外基金
     ——如实告知不支持，不得反复重试或编造数据。
```
...
async def _execute(task_id: str, input_text: str, budget: TaskBudget,
    ...
    # 2021-2024（训练截止时钟），与真实区间偏移两年。注入日期锚点，
    # 相对日期一律以它解析（与 Memory 注入同一消息位，事实仍以工具为准）。
    # v32：相对区间按自然月对齐（忽略进行中的当月、端点对齐月初/月末）——
    # 滚动日界窗口（如 2023-08-17~2026-08-17）每日漂移击穿缓存；月对齐后
    # 区间按月稳定，且语义不变（今天 2026-08-17 的"近三年"=2023-08-01至2026-07-31）。
    date_line = (f"今天是 {dt.date.today().isoformat()}。"
                 f"用户输入中的相对时间（如\"近三年\"）必须以该日期解析，"
                 "并按自然月对齐：忽略进行中的当月，区间端点对齐到月初/月末"
                 "（例：今天是2026-08-17，则\"近三年\"=2023-08-01至2026-07-31，"
